        ...
        callback(
            ...
            move |_, (u, l): (Option<u64>, Option<u64>)| {
                ...
                match (u, l) {
                    ...
                    (Some(high), Some(low)) => {
                        // SmallRng's seed is 32 bytes on 64-bit targets but 16
                        // bytes on wasm32. Fold both Lua seed words into the
                        // portable SeedableRng entry point instead of assuming
                        // one target's associated Seed type.
                        let seed = low ^ high.rotate_left(32);
                        *rng.borrow_mut() = SmallRng::seed_from_u64(seed);
                        Some(())
                    }
                    _ => None,
